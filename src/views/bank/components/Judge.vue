...
onMounted(() => {
    ...
    judgeStore.getJudgeListAction(Number(bankId.value))
...
const handleSubmit = () => {
  ...
  if (!judgeStore.isFinished) {
    showConfirmDialog({
      title: '温馨提示',
      message:
        `你还有 ${judgeStore.leftQuestion} 道未完成的题目, 确定交卷?`,
    })
      .then(async () => {
        // 发送提交请求
        await judgeStore.addFinishedQuestion({
          bankId: +bankId.value,
          sort: '判断题',
          correctList: judgeStore.correctList as any,
        })
        router.push(`/bank/${bankId.value}/judge_submit`)
...
<template>
  <Navbar title="判断题"  left-text="题型分类" :left-arrow="true" :left-fn="backToSort"/>
  ...
  <div id="judge" class="judge">
    ...
    <!-- 题目区域 -->
    <div v-if="judgeStore.judgeList?.length" class="page-wrapper">
        ...
        <div v-show="index === currentCardIndex" class="item" :id="index + ''">
          <div class="title">
            <div class="question-desc-header">
              <div class="commonClass judgeClass">判断题</div>
